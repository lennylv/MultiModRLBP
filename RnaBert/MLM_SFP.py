...
parser.add_argument('--maskrate', '-m', type=float, default=0.0,
                    help='mask rate')
parser.add_argument('--pretraining_bert', '-pre_bert', type=str, help='use pretrained weight')
    ...
    def test(self, ds, test_loader, model):
        ...
        for i in range(data_num):
            ...
            for data0, data1 in zip( test_loader, single_seq):
                ...
                x0, label0 = x0.to(self.device),label0.to(self.device),
                x1, label1 = x1.to(self.device),label1.to(self.device),
